...
function applyTrackers(_anonID) {
	...
	// Send initial event 
	tracker.sendEvent('panIDPing_v134', _anonID,  Date.now());

	// Ping GA every half an hour, with anonymized random ID to show how many active users we really have
	window.setInterval(function() {tracker.sendEvent('panIDPing_v134', _anonID,  Date.now());},  1000*60*30);
}
	...
	chrome.storage.sync.get('anonID',
							function(items) {
								...
								if (anonID && anonID.length > 0 && anonID.startsWith('aid_')) {
									...
									tracker.sendEvent('panIDPing_v134', 'AID regenerated',  Date.now());
